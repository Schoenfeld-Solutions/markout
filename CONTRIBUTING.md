...
- `npm run dev` to run the repo-native webpack watch/static server plus manual
  **Add from File** sideload with `manifest-localhost.xml`; configure
  `MARKOUT_DEV_TLS_CERT_PATH` and `MARKOUT_DEV_TLS_KEY_PATH` when the default
  local certificate files do not already exist
